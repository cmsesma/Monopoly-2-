...
Monopoly::GameState::GameState(const std::string& RulesFileName,
    ...
    : rules(RulesFileName),
      board(BoardFileName, *this, rules),
...
const Monopoly::Rules Monopoly::GameState::getRules(){
  return rules;
...
void Monopoly::GameState::rollDice(Monopoly::Player& player) {
  ...
  if(player.in_jail_or_not == 1){
    ...
    player.giveCash(this->board.getGoSpace().getSalary() * timesPastGo);
...
Monopoly::Board& Monopoly::GameState::getBoard() {//i added
  return board;
}
