...
Monopoly::Space::Space(std::string space_type_string, CSVReader& boardFile, const int spaceNumber, GameState& gameState)
    : spaceNumber(spaceNumber), space_type_string(space_type_string), gameState(gameState) {
  //std::string space_type_string = boardFile.get_next_field();

  //std::cout << "PRINTING IN space.cpp----" << space_type_string << std::endl;
  ...
  if (space_type_string == "Go") {
    ...
    //std::cout << space_type_string << std::endl;
...
std::string Monopoly::Space::getspace_type_string() const {
  return space_type_string;
}
/*
void Monopoly::Space::activate(Monopoly::Player& activatingPlayer) { //need to edit for new space types
  if (spaceType == SpaceType::goSpace) {
